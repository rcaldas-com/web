...
export default function DigitarClient({ canUseExternalAi }: { canUseExternalAi: boolean }) {
  ...
  const [enginePreference, setEnginePreference] = useState<'tesseract' | 'openai'>('tesseract');
  ...
  const onProcess = async () => {
    ...
    try {
      ...
      formData.append('engine', canUseExternalAi ? enginePreference : 'tesseract');
          ...
          {canUseExternalAi && (
            <label className="flex items-center gap-2 rounded-lg border border-zinc-300 px-3 py-2 text-sm text-zinc-700">
              <input
                ...
                onChange={e => setEnginePreference(e.target.checked ? 'openai' : 'tesseract')}
