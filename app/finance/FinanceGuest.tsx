...
export default function FinanceGuest() {
  ...
  const expenses = getAllLocalExpenses();
  ...
  const projections = Array.from({ length: 6 }, (_, i) => {
    const pym = addMonthsToYearMonth(yearMonth, i + 1);
    const pDays = daysInYearMonth(pym);
    const pOverrides = getLocalExpenseOverrides(pym);
    const ig = groupInstallments(installments, cards, Math.max(0, monthOffset + i + 1));
    const calc = calculateMonthBalance(profile, filterExpensesForMonth(expenses, pym), ig, pDays, pOverrides);
