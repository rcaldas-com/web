...
if [[ "$ENABLE_TUNNEL" == "true" ]] && ! command -v rsyslogd >/dev/null 2>&1; then
  ...
  DEBIAN_FRONTEND=noninteractive apt-get -qq install rsyslog > /dev/null 2>&1 ||     echo "  AVISO: nao consegui instalar rsyslog -- logs nao serao centralizados"
fi
...
if [[ -n "$wanted_port" && -z "$active_port" && -n "$local_ssh_port" ]]; then
  ...
  ssh -i /root/.ssh/id_ed25519 -o UserKnownHostsFile=/etc/rcaldas-agent/known_hosts -o StrictHostKeyChecking=accept-new       -fNR "$wanted_port:127.0.0.1:$local_ssh_port"       -L "$LOG_FORWARD_PORT:127.0.0.1:514"       -p "$TUNNEL_RELAY_PORT" "zxnet@$TUNNEL_RELAY" 2>>"$LOG"       || log "falha ao abrir tunel na porta $wanted_port"
...
if printf '%s' "$response" | grep -q '"hasJobs":true'; then
  jobs=$(curl -fsS -m 20 -H 'Content-Type: application/json' -X POST "$APP_URL/agent-jobs"     -d "{\"host\":\"$(json_escape "$HOST_NAME")\",\"token\":\"$(json_escape "$AGENT_TOKEN")\"}" 2>/dev/null || true)
  job_results=""
