...
for conf in /etc/rsnapshot/*.conf; do
  ...
  # Nem todo .conf define todo intervalo: os de backup de DADOS nao tem
  # "hora" de proposito -- o menor nivel e' quem executa o dump de verdade,
  # e nem bater no Mongo nem varrer o bucket de 4 em 4 horas se paga. Sem
  # esta checagem o rsnapshot aborta com "interval hora not defined" e a
  # falha vira alarme, que e' ruido: nao ha nada errado em nao rodar.
  if ! grep -qE "^retain[[:space:]]+$INTERVALO[[:space:]]" "$conf"; then
    continue
  fi
  inicio=$(date +%s)
...
if [[ "$INTERVALO" == "dia" ]] && command -v restic >/dev/null 2>&1; then
  if [[ -f "$CONF_DIR/s3.env" && -f "$CONF_DIR/restic-pass" ]]; then
    ...
    for conf in /etc/rsnapshot/*.conf; do
      ...
      # O menor nivel definido no .conf e' o que tem o dado mais fresco:
      # "hora" pros hosts, "dia" pros .conf de dados (que nao tem hora).
      # Fixar "hora.0" aqui deixava o backup de dados fora do offsite.
      raiz_conf=$(grep -E "^snapshot_root[[:space:]]" "$conf" | awk '{print $2}' | sed 's|/$||')
      [[ -n "$raiz_conf" ]] || raiz_conf="$raiz/$h"
      for nivel in hora dia semana mes; do
        if grep -qE "^retain[[:space:]]+$nivel[[:space:]]" "$conf"; then
          [[ -d "$raiz_conf/$nivel.0" ]] && fontes+=("$raiz_conf/$nivel.0")
          break
        fi
      done
